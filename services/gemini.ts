...
export const generateAudioPreview = async (t: string): Promise<AudioBuffer> => {
  try {
    ...
    // GRACEFUL FALLBACK: Create a silent placeholder when API limit is hit
    // This allows the app to continue functioning in demo mode
    console.warn("Audio preview unavailable (API limit reached). Using silent placeholder.");
    const audioContext = new (window.AudioContext || (window as any).webkitAudioContext)({ sampleRate: 24000 });
    // Create 0.5 second silent buffer as placeholder
    const silentBuffer = audioContext.createBuffer(1, 12000, 24000);
    throw new Error("Preview Unavailable (API Limit)"); // Still throw to show user feedback
  }
};
